...
export const deploy = async (subgraphType: string): Promise<void> => {
  ...
  try {
    let deployCommand = `graph deploy --node ${node} --ipfs ${ipfs} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`
    if (deployKey) {
      deployCommand = `graph deploy --node ${node} --ipfs ${ipfs} --deploy-key ${deployKey} --version-label ${gitHashString} ${subgraphName} ${subgraphType}-subgraph.yaml`
    }
